...
# custom function pour enregistrer l'activité
def record_tokens_usage_v1(query, context, output, model, price):
    ""
    tokens_counter = tiktoken.encoding_for_model("gpt-4o-mini")            
    tokens_in= (tokens_counter.encode(query+context))
    tokens_out= (tokens_counter.encode(output))
    
    hash_query = md5(query.encode()).hexdigest()

    in_price=out_price=0
    if isinstance(price, dict) and len(price.items())>0:
        try:
            in_price=(price["in"]*len(tokens_in))/1000000
            out_price=(price["out"]*len(tokens_out))/1000000
        except Exception as e:
            print(e, "llm price not defined")
    record={
        "graph": {
            hash_query: {
                "query": query,
    ...
    WORKING_DIR = SCRIPT_DIR.parent/"logs"

    curr_path=WORKING_DIR /"tokens_usage_graph.json"
...
def record_tokens_usage(
    ...
    input_price= completion_price= reasoning_price=0
    if len(pricing.items())==2:
        input_price=(pricing["input"] * input_tokens)/1000000
        completion_price=(pricing["output"] * completion_tokens)/1000000
        reasoning_price=(pricing["output"] * reasoning_tokens)/1000000        
    ...
    record={
        ...
        "input_price": round(input_price, 5),
        "completion_price": round(completion_price, 5),
        "reasoning_price": round(reasoning_price, 5),
        'model': model,
    }
    ...
    FileManager.write(curr_path, record, "append")
...
def limit_async_func_call(max_size: int, waitting_time: float = 0.0001):
    ...
    def final_decro(func):
        ...
        async def wait_func(*args, **kwargs):
            ...
            #         context+=kwargs["system_prompt"]

            #     record_tokens_usage(query=args[0], context=context, output=result)
