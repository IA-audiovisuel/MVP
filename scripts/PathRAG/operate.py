    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        for now_glean_index in range(entity_extract_max_gleaning):
            ...
            if glean_result is None:
                glean_result='{"entities": [], "relationships": [], "content_keywords": []}'
            elif final_result is None:
                final_result='{"entities": [], "relationships": [], "content_keywords": []}'
            ...
            try:
                ...
                print(e)
                # if final_result is None and glean_result is None:
                #     final_result = ""
                # elif final_result is None and glean_result is not None:
                #     final_result=glean_result
...
async def kg_query(
    ...
    def inject_metadata_to_headers(llm_func, query_param, query):
        ...
        extra_headers = dict(p.keywords.get("extra_headers", {}))
        extra_headers["user_query"] = query
        p.keywords["extra_headers"] = extra_headers


        # ajout id de la requete
        if "hash_query" in query_param.__dataclass_fields__ :
            extra_body = dict(p.keywords.get("extra_body", {}))
            extra_body["user"] = f"audio-graphrag-qa-{query_param.hash_query}"
            p.keywords["extra_body"] = extra_body

            extra_headers = dict(p.keywords.get("extra_headers", {}))
            extra_headers["hash_query"] = query_param.hash_query
            p.keywords["extra_headers"] = extra_headers



        # ajout prix llm
        if "llm_price" in query_param.__dataclass_fields__:
            extra_headers = dict(p.keywords.get("extra_headers", {}))
            extra_headers["llm_pricing"] = json.dumps(query_param.llm_price)
            p.keywords["extra_headers"] = extra_headers


    # custom code to trace activity
    inject_metadata_to_headers(use_model_func, query_param, query)
