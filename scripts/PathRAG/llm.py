...
async def openrouter_complete(
    ...
    **kwargs
    ...
    llm_price= None
    # if "llm_price" in kwargs["extra_body"]:
    #     llm_price= kwargs["extra_body"]["llm_price"]
    #     del kwargs["extra_body"]["llm_price"]
...
async def openai_complete_if_cache(
    ...
    **kwargs,
    ...
    # ajout pour tracer activité
    if extra_headers is not None and "user_query" in extra_headers:
        user_query=extra_headers["user_query"]   
        del extra_headers["user_query"]
    else:
        user_query= ""
    ...
    record_tokens_usage(
        ...
        model=model, 
        pricing=json.loads(extra_headers["llm_pricing"]) if "llm_pricing" in extra_headers  else {}
    )


    if user_query!="":
        extra_headers["user_query"]=user_query
    ...
    if hasattr(response, "__aiter__"):
        ...
        hash_query = md5((user_query+model).encode()).hexdigest()
        context_saving={
            "hash_query": hash_query,
            "query": user_query,
            "date": datetime.datetime.now().isoformat(),
            "context": f"system_pompt:\n{system_prompt}\n\nuser_prompt:\n{prompt}",
            "response": content,
            "llm": model
        }
        
        FileManager.write("context_used/pathrag.json", context_saving, "append")
