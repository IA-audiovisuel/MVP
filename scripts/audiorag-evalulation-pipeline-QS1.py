...
async def main(filename: str, DOC_NAME:str, CONFIG: dict, TOKENS_USAGE: str):
    ...
    class HuggingFaceInferenceEmbeddings(Embeddings):
        """Wrapper LangChain-compatible autour de l'API HuggingFace Inference."""
    ...
    def normalize_chroma_name(raw_name: str) -> str:
        # 1. Supprime les accents et caractères non-ASCII (é -> e, à -> a, etc.)
        normalized = unicodedata.normalize('NFKD', raw_name)
        ascii_name = normalized.encode('ascii', 'ignore').decode('ascii')
        
        # 2. Remplace tout caractère non autorisé par un tiret
        cleaned = re.sub(r'[^a-zA-Z0-9._-]', '-', ascii_name)
        
        # 3. Évite les séparateurs consécutifs (--- -> -)
        cleaned = re.sub(r'[-_.]+', '-', cleaned)
        
        # 4. Garantit le début et la fin alphanumériques
        cleaned = cleaned.strip('-_.')
        
        # 5. Longueur minimale (3)
        if len(cleaned) < 3:
            cleaned = (cleaned + "000")[:3]
            
        # 6. Troncature à 512 caractères (sans finir par un séparateur)
        if len(cleaned) > 512:
            cleaned = cleaned[:512].rstrip('-_.')
            
        # 7. Fallback sécurisé si jamais la chaîne reste invalide
        if not re.match(r'^[a-zA-Z0-9][a-zA-Z0-9._-]*[a-zA-Z0-9]$', cleaned):
            cleaned = f"doc_{hashlib.md5(raw_name.encode()).hexdigest()[:8]}"
    ...
    def create_vectorDB(filename: str, DOC_NAME_hybrid: str):
        ...
        # DOC_NAME_cleaned = DOC_NAME_hybrid.replace(" - ", "-").replace(" ", "-").replace(",", "-").lower()
        DOC_NAME_cleaned=normalize_chroma_name(DOC_NAME_hybrid)
        persist_dir = f'./storage/vector_scores/{DOC_NAME_cleaned}'

        if not rebuild_vectorStore:
            return
        ...
        embeddings = HuggingFaceInferenceEmbeddings(model="intfloat/multilingual-e5-large")
    ...
    class RAG_hybrid():
        def __init__(self, model, DOC_NAME):
            ...
            self.DOC_NAME_hybrid = normalize_chroma_name(DOC_NAME)
    ...
    async def llm_as_judge(question: str, question_type: str, reference: str, candidat: str, llm_config: str) -> str:
        ...
        judges_results=[]
        for full_resp, llm in zip(full_responses, llm_config["judges"]):

            resp=full_resp.choices[0].message.content
            # spécifique à deepseek 3.2 via openrouter
            if resp is None:
                resp= full_resp.choices[0].message.reasoning            
            try:
    ...
    for rag in rag_pipelines:
        ...
        for el in range(0, len(DATASET_TEST) ):
            ...
            query_type= DATASET_TEST[el]["source"],
...
# Params Obligatoires
DOC_NAME=filename="Silicon Valley - Les milliardaires contre la démocratie.txt"
DOC_NAME=DOC_NAME.replace(".txt", "")
corpus_distilled="distilled_Silicon Valley - Les milliardaires contre la démocratie.txt_by_gemini-3.1-pro-preview.md" #-> produire avec 'corpus_distillation.py'
...
# success:
# Qwen/Qwen3.5-35B-A3B:novita "pricing": {"input": 0.25, "output": 2}
# Qwen/Qwen3-235B-A22B-Instruct-2507 "pricing": {"input": 0.09, "output": 0.58}
# Qwen/Qwen3-30B-A3B:novita "pricing": {"input": 0.09, "output": 0.45}
# Qwen/Qwen3-Next-80B-A3B-Instruct:together     "pricing": {"input": 0.15, "output": 1.5}
# Qwen/Qwen3-Next-80B-A3B-Instruct:hyperbolic "pricing": {"input": 0.3, "output": 0.3}
# XiaomiMiMo/MiMo-V2-Flash:novita "pricing": {"input": 0.1, "output": 0.3}
# meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8:novita "pricing": {"input": 0.27, "output": 0.85}
...
# model_id="openai/gpt-oss-120b"information_
# model_id="mistralai/ministral-8b-2512"
# model_id="z-ai/glm-4.6"
# model_id="moonshotai/kimi-k2"

DATASET_TEST=FileManager.read(path="validation_questions/questions-specfiques-QS1-Silicon Valley - Les milliardaires contre la démocratie_gemini-3.1-pro-preview.json")
...
CONFIG={        
    ...
    "filename_output_evaluations": "evaluations/evaluations_results_pipeline_QS1.json",
