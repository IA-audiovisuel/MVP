...
async def main(filename: str, DOC_NAME:str, CONFIG: dict, TOKENS_USAGE: str):
    ...
    async def llm_as_judge(question: str, question_type: str, reference: str, candidat: str, llm_config: str) -> str:
        ...
        try:
            system_prompt="""
                ...
                ### NEEDLE
                Question portant sur une information très précise du corpus.
                - B doit restituer l'information exacte sans approximation
                - Une réponse correcte mais vague est insuffisante
                - Une information absente de A mais précise et cohérente avec A est tolérée

                ### MULTI_HOP
                Question nécessitant de relier plusieurs informations du corpus.
                - B doit démontrer la connexion entre les éléments, pas seulement les lister
                - Un raisonnement intermédiaire visible (même procédural) est valorisé, pas pénalisé
                - L'absence d'un des sauts logiques est une limite à signaler

                ### QUOTE_EXPLANATION
                Question demandant d'expliquer ou clarifier une phrase marquante du corpus.
                - B doit restituer fidèlement le sens de la citation dans son contexte
                - Une reformulation cohérente avec A est acceptable même si différente en surface
                - Ne pas pénaliser B si elle enrichit l'explication au-delà de A, tant que cohérent

                ### CONTEXTUAL_TRAP
                Question dont la réponse correcte dépend spécifiquement du corpus, même si elle contredit l'intuition générale.
                - B doit privilégier ce que dit le corpus sur ce que suggère la connaissance générale
                - Si B répond selon l'intuition générale en contradiction avec A → erreur critique
                - Si B suit le corpus contre-intuitif → valoriser explicitement

                ### FALSE_PREMISE
                Question contenant une prémisse incorrecte ou un sujet non abordé dans le corpus.
                - B doit identifier et corriger la prémisse, ou signaler que le sujet est absent
                - Une réponse qui accepte la prémisse sans la questionner est une erreur critique
                - Un refus bien motivé est préférable à une réponse construite sur une base fausse
                ...
                ## CAS PARTICULIER — FALSE_PREMISE : évaluation des refus
                ...
                le RAG a correctement détecté l'insuffisance de contexte ou la fausse prémisse.

                Ce comportement procédural NE DOIT PAS être pénalisé sur le style.

                Ce qui compte uniquement :
                - B a-t-elle correctement identifié que le contexte est insuffisant ou la prémisse fausse ? → valoriser
                - B a-t-elle inventé une réponse malgré l'absence de contexte ? → pénaliser
                - B a-t-elle accepté une fausse prémisse sans la questionner ? → erreur critique
                ...
                que A, dès lors que la détection est correcte et qu'aucune information n'a été inventée.
                ...
                - Pour FALSE_PREMISE : qualité de la gestion de l'incertitude ou de la prémisse
            ...
            return {"judge": "", "score_A": 0, "score_B": 0, "evaluation": "call error"}
    ...
    for rag in rag_pipelines:
        ...
        for el in range(0, len(DATASET_TEST) ):
            ...
            query_type= DATASET_TEST[el]["type"],
            ...
            if len(evaluation.items())>0:
                evaluations_results={
                        ...
                        "dataset": "QS2",
                        "question": DATASET_TEST[el]["question"],                    
                        "type": DATASET_TEST[el]["type"],
                        "difficulte": DATASET_TEST[el]["difficulte"],
                        "source": DATASET_TEST[el]["source"],
                        "reponse_reference": DATASET_TEST[el]["reponse_reference"],                    
...
DATASET_TEST=FileManager.read(path=SCRIPT_DIR / "validation_questions/questions-specifiques+false premise-QS2-La France est-elle reformable.json")
...
CONFIG={        
    ...
    "filename_output_evaluations": "evaluations_results_pipeline_QS2.json",
